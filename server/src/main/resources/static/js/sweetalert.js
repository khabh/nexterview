        ...
        mn = e => {
            e.container.onmousedown = t => {
                t.target === e.container && t.preventDefault(), e.popup.onmouseup = function (t) {
                    e.popup.onmouseup = () => {
                    }, (t.target === e.popup || t.target instanceof HTMLElement && e.popup.contains(t.target)) && (dn = !0)
                }
            }
        },
        hn = (e, t, n) => {
...
async function apiFetch(url, options = {}) {
    try {
        ...
        if (!response.ok) {
            const errorMessage = `🚨 오류 발생: HTTP ${response.status}`;
            Swal.fire({
                title: "오류 발생!",
                text: errorMessage,
                icon: "error",
        ...
        const errorMessage = "⚠️ 네트워크 오류 또는 서버에 접근할 수 없습니다.";
        Swal.fire({
            title: "네트워크 오류!",
            text: errorMessage,
            icon: "warning",
